...
class ExpensesTracker:
    ...
    def inject_malicious_code(self):
        # Vulnerabilidad: Referencia directa insegura
        index = random.randint(0, len(self.expenses)-1)
        expense = self.expenses[index]
        # Devolviendo los detalles completos de un gasto aleatorio
        return expense
...
def home():
    ...
    # Obtener un gasto aleatorio para mostrar en la página
    expense_expuesto = tracker.inject_malicious_code()
    
    return render_template('index.html', reporte=reporte, expense_expuesto=expense_expuesto)
